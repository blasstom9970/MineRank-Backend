...
def posts():
    if request.method == "POST":
        ...
        null_posts = cursor.fetchall()
        
        if null_posts:
            # 다음 사용 가능한 ID 찾기
            cursor.execute("SELECT COALESCE(MAX(id), 0) FROM community_posts WHERE id IS NOT NULL")
            max_result = cursor.fetchone()
            next_id = (max_result[0] if max_result else 0) + 1
            
            # null ID 게시물 삭제 후 올바른 ID로 재삽입
